...
- Decision: Issue #3 이후 domain rule 변경은 `tdd` skill을 명시적으로 호출해 red-green-refactor loop로 진행한다.
  Rationale: #2에서는 Edit Mode tests를 추가했지만 `tdd` skill을 명시적으로 사용하지 않아 다음 작업자가 test-first 의도를 놓칠 수 있다. Assignment validation, day resolution, victory/failure는 domain behavior이므로 public API를 통한 behavior test를 먼저 만들고, Unity UI는 domain이 green 상태일 때 얇게 연결하는 편이 안전하다.
...
Issue #3부터 domain rule을 바꾸는 작업자는 구현 전에 `tdd` skill을 호출한다. 한 번에 모든 테스트를 쓰지 말고, public API로 관찰 가능한 behavior 하나를 고르는 vertical slice로 진행한다. 먼저 해당 behavior의 Edit Mode test를 추가하고 실패를 확인한 뒤, 그 테스트를 통과시키는 최소 domain code만 구현한다. 모든 domain tests가 green이 된 뒤에 Unity Runtime/UI adapter를 수정한다.
...
Domain acceptance는 Edit Mode tests로 판단한다. `SafeVillage.Core.Tests`가 모두 통과해야 하고, tests는 success path, wall failure path, food failure path를 모두 고정해야 한다. Domain rule 변경은 `tdd` skill의 red-green-refactor loop를 따른다. 각 test는 implementation detail이 아니라 `VillageGame` 같은 public interface로 확인되는 behavior를 설명해야 하며, domain tests가 실패하면 UI 구현으로 넘어가지 않는다.
...
Issue #4 구현 산출물은 다음과 같다.

    Assets/Tests/EditMode/SafeVillage.Core.Tests/VillageGameTests.cs

Issue #4 검증 결과는 다음과 같다.
    ...
    unity-mcp-cli run-tool script-execute --input '{...SafeVillageIssue4CleanProbe...}'
    State text:
      Safe Village Micro
      Day: 3
      Food: 0
