...
- 001-015 프롬프트 및 012a/013a 실습 프롬프트: 튜터 모드
- 016-018 프롬프트: 일반 모드
...
3. 실험 실행 단계에서는 Codex가 기본적으로 파일을 생성/수정하지 않는다. 단, 012a/013a 실습 프롬프트는 운영 규칙의 실습 예외를 따른다.
...
10. `012a-mcp-mock-tool.md`는 `012-mcp-concepts` 결과 기록 이후 실행한다.
11. `013a-plugin-bundle-practice.md`는 `013-plugin-concepts` 결과 기록 이후 실행한다.
12. `018-ai-gap-map.md`는 `017-final-selection` 결과 기록 이후 실행한다.
...
Safe Village Micro 구현으로 넘어가지 마라.
파일을 생성/수정하지 마라.
단, 012a/013a 실습 프롬프트가 명시한 tmp/prompts-lab 하위 실습 경로는 예외로 허용한다.
현재 repo 구조를 먼저 확인하고, 확인된 사실과 추측을 분리해서 답해라.
Unity gameplay code, Scene, Packages/manifest.json, ProjectSettings/는 건드리지 마라.
...
012a/013a처럼 suffix가 있는 실습 프롬프트는 같은 suffix를 결과 파일명에도 유지해줘.
...
- 일반 결과 파일명은 프롬프트 파일과 같은 번호와 topic name을 사용한다. `012a`, `013a` 같은 suffix도 유지한다.
...
- 단, `012a-mcp-mock-tool.md`와 `013a-plugin-bundle-practice.md`는 실습 프롬프트이므로 각 파일이 명시한 `tmp/prompts-lab/...` 하위 경로에만 실습 파일 생성을 허용한다.
