...
- 001-015 프롬프트 및 006a/012a/013a 실습 프롬프트: 튜터 모드
- 016-018 프롬프트: 일반 모드
...
1. `docs/prompts/NNN-topic-name.md` 또는 `docs/prompts/NNNa-topic-name.md`에서 실행할 프롬프트를 고른다.
2. Codex에게 `이 파일을 튜터 모드로 실행해줘` 또는 `이 파일을 일반 모드로 실행해줘`라고 요청한다.
3. 실험 실행 단계에서는 Codex가 기본적으로 파일을 생성/수정하지 않는다. 단, 006a/012a/013a 실습 프롬프트는 운영 규칙의 실습 예외를 따른다.
...
10. `006a-hooks-guard-practice.md`는 `006-hooks-concepts` 결과 기록 이후 실행한다.
11. `012a-mcp-mock-tool.md`는 `012-mcp-concepts` 결과 기록 이후 실행한다.
12. `013a-plugin-bundle-practice.md`는 `013-plugin-concepts` 결과 기록 이후 실행한다.
13. `018-ai-gap-map.md`는 `017-final-selection` 결과 기록 이후 실행한다.
...
Safe Village Micro 구현으로 넘어가지 마라.
파일을 생성/수정하지 마라.
단, 006a/012a/013a 실습 프롬프트가 명시한 tmp/prompts-lab 하위 실습 경로는 예외로 허용한다.
현재 repo 구조를 먼저 확인하고, 확인된 사실과 추측을 분리해서 답해라.
Unity gameplay code, Scene, Packages/manifest.json, ProjectSettings/는 건드리지 마라.
...
006a/012a/013a처럼 suffix가 있는 실습 프롬프트는 같은 suffix를 결과 파일명에도 유지해줘.
...
- 일반 결과 파일명은 프롬프트 파일과 같은 번호와 topic name을 사용한다. `006a`, `012a`, `013a` 같은 suffix도 유지한다.
...
- 단, `006a-hooks-guard-practice.md`, `012a-mcp-mock-tool.md`, `013a-plugin-bundle-practice.md`는 실습 프롬프트이므로 각 파일이 명시한 `tmp/prompts-lab/...` 하위 경로에만 실습 파일 생성을 허용한다.
- `실행하고 기록해줘` 같은 결합 요청도 먼저 파일 수정 없이 실행하고, 그 뒤 결과 기록 단계에서만 파일을 수정한다.
- 결과 기록 파일을 만들 때만 `docs/prompt-results/` 수정이 허용된다.
- 일반 실험은 `docs/prompt-results/NNN-topic-name.md`에 요약 기록을 남긴다. suffix가 있는 실습 프롬프트는 같은 suffix를 유지한다.
...
- 실습 프롬프트에서도 `.codex/`, global config, plugin registry, marketplace 파일은 별도 승인 전 수정하지 않는다.
- 실습 프롬프트는 dependency 설치, 네트워크 사용, 실제 Unity Editor 연동을 기본적으로 하지 않는다.
...
| 006a | 튜터 | `docs/prompts/006a-hooks-guard-practice.md` | `docs/prompt-results/006a-hooks-guard-practice.md` |
...
| 012a | 튜터 | `docs/prompts/012a-mcp-mock-tool.md` | `docs/prompt-results/012a-mcp-mock-tool.md` |
| 013 | 튜터 | `docs/prompts/013-plugin-concepts.md` | `docs/prompt-results/013-plugin-concepts.md` |
| 013a | 튜터 | `docs/prompts/013a-plugin-bundle-practice.md` | `docs/prompt-results/013a-plugin-bundle-practice.md` |
